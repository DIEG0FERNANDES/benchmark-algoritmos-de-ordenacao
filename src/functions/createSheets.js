...
const createSheets = () => {
  ...
  const header = [
    'Método de ordenação',
    ...['primeira', 'segunda', 'terceira', 'quarta', 'quinta'].map(value => `Tempo da ${value} execução (ms)`),
    'Média aritmética'
  ...
  runTimes.forEach(runtime => {
    ...
    if (runtimeArray.length === 9) {
      const [milliseconds, seconds, minutes, hours] = runtimeArray[7].split(/[:\.]/).reverse().map(value => Number(value));
  ...
  for (let i = 0; i < 4; ++i) {
    for (const rawSortingMethod of Object.keys(sortingMethods)) {
      ...
      for (let j = 1; j <= 5; ++j) {
        const label = { sortingMethod: rawSortingMethod, entry: (i + 1), loop: j };
    ...
    filePath = path.join(__dirname, '..', 'sheets', `results-over-entry-${(i + 1)}.csv`);
